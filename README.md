...
Python has been around a while (since the early 90s) and so its many ways of managing executable versions and python packages has evolved into a confusing ecosystem of competing tools (`virtualenv`, `pipenv`, `conda`, etc.) each claiming to be 'the right way'.

The approach taken here revolves around usage of what seems to me to be the most long-lived/common means of running python environments, which is `virtualenv`. Being the most widespread python virtual-environment executable, I highly recommend you memorize the following sequence as part of your python know-how:
...
Sourcing the virtualenv activation script simply adjusts your working shell by adding/altering the following env variables:
...
For the above to work, you obviously need to have `virtualenv` and a python-v3 interpreter installed; if you're on a Mac then I recommend installing python via homebrew: `brew install conda`. This will install the latest version of python3 (3.7 at this moment in time) and pip3. Once pip3 is installed, use it to install virtualenv with `pip3 install virtualenv`.

Note: I recommend you use python, pip and virtualenv from the same source. If, by example, you install python using the anaconda application then use its python, pip and virtualenv in your project; if you use conda's virtualenv with a different python interpreter then you might get errors.

#### Initialization Script

This repo includes a script called `_initial_setup.sh` that makes it easy and consistent to start up your python virtual environment. The first thing it does is to load in the contents of the `.env` file in order to locate the `virtualenv` and `python3` executables on your machine that you want to use within your activated virtual environment.

(The `.env` file is not committed to the git repo since, in general, it will contain passwords and directory structures that you want to keep confidential, so copy `.env-template` to `.env` and fill in the paths for `PYTHON_3_5_OR_HIGHER` and `VIRTUALENV` on your machine.)

`_initial_setup.sh` is designed to fail unless it is sourced from your command line (not executed with e.g. `sh`). Once it creates the python virtual environment and activates it, it upgrades pip and installs packages specified in `requirements.txt`.

Running `source _initial_setup.sh` is idempotent, so get in the habit of running regularly whenever you want to get up and running with your development.

#### Adding Packages

As you develop within your virtual environment, you'll be adding python packages using `pip install PACKAGENAME` which will install the package to `.venv`. To make a permanent record of an installed package, run `sh _freeze_requirements` to get it added to `requirements.txt`.

#### Directory Structure

All scripts and configuration files are in the root directory. The source code is in `src`. Once the virtual environment is activated you run your application using `_run_app.sh` (a simple wrapper around `python src/main.py`).
...
You do NOT want to spend time/effort lining up code blocks, tabbing brackets, etc. while coding. By getting your editor to automatically format your code according to industry standards, you'll save time and make it easier to collaborate with others. To enable autoformatting in VSCode:
